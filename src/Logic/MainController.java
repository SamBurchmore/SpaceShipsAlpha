...
public class MainController {
    ...
    private Color darkTeamMemberColor = new Color(180, 180, 0);
        ...
        public void runGame() throws IOException, InterruptedException {
            ...
            playLogic.getPointer(); // Get the mouse location
                ...
                if (gameState == GameState.WAITING_INPUT) {
                    ...
                    if (pieceSelected) {
                        attackCost = activePiece.getActionCost();
                ...
                } else if (activePiece == lastClickedTile.getPiece()) {
                    resetTurn();
                }
                else if (lastClickedTile.getPiece() != null) {
                    if (lastClickedTile.getPiece().getTeam() == activeTeam) { // Check if player is selecting another piece
                        if (!teamMemberTiles.isEmpty() && gameState != GameState.MULTIPLE_PIECES_SELECTED) {
                            for (Tile teamMember : teamMemberTiles.keySet()) {
                                if (lastClickedTile.getPiece() == teamMember.getPiece() && !lastClickedTile.getPiece().hasMoved()) {
                                    ...
                                    attackCost += teamMember.getPiece().getActionCost();
                                    System.out.println(attackCost);
                                    ...
                                    for (Tile teamMemberTile : teamMemberTiles.keySet()) {
                                        for (Tile enemyTile : teamMemberTiles.get(teamMemberTile)) { // Remove team members not in range of any enemies in the new attackers range or that cost to many action points
                                            for (Tile currentTeamMemberAttackTile : teamMemberTiles.get(teamMember)) {
                                                if (currentTeamMemberAttackTile.getPiece() == enemyTile.getPiece() && attackCost <= maxActionPoints) {
                ...
                if (gameState == GameState.MULTIPLE_PIECES_SELECTED || gameState == GameState.SELECTING_MORE_PIECES) {
                    boolean actionPerformed = boardLogic.performMultiAttack();
                    if (actionPerformed) {
    ...
    public class BoardLogic {
        ...
        public ArrayList<Object> multiAttack(Set<Tile> attackers, Piece defender) {
            ...
            for (Tile attacker : attackers) {
                if (attacker.getPiece().getType() == PieceType.FRIGATE) {
                    ...
                    attackScore += 1 + random.nextInt(attacker.getPiece().getAttack());
                }
        ...
        public boolean performMultiAttack() {
            if (activePiece.hasMoved()) { // If piece has already been moved this turn, do nothing and return false
                return false;
            }
            for (Tile tile : attackTiles) { // Iterate over potential attack tiles, if one matches the last clicked tile, run attack() and return true
        ...
        public HashMap<Tile, ArrayList<Tile>> getTeamMembersInAttackRangeOfTarget(Team team, ArrayList<Tile> enemyTiles, Piece activePiece) {
            ...
            for (Tile teamMember : aliveTeamMembers) {
                int actionPointCost = teamMember.getPiece().getActionCost();
                ...
                for (Tile enemyTileInRange : enemiesTilesInRange) {
                    for (Tile enemyTile : enemyTiles) {
                        if (enemyTileInRange.getPiece().getID() == enemyTile.getPiece().getID() && activePiece.getID() != teamMember.getPiece().getID() && activePiece.getActionCost() + attackCost + teamMember.getPiece().getActionCost() <= maxActionPoints && !teamMember.getPiece().hasMoved()) {
    ...
    public class BoardImage {
        ...
        public BufferedImage getMiniImage() {
            BufferedImage miniImage = blankBoard(15);
            int size = boardController.getSize();
            int scale = 15;
            for (int x = 0; x <= scale * size; x += scale) {
                for (int y = 0; y <= scale * size; y += scale) {
